...
export function Chatbox({ onSavePlanToSchedule }: ChatboxProps) {
  ...
  return (
    <div className="flex h-screen bg-bg-dark overflow-hidden relative">
      ...
      <div className={`
        fixed inset-y-0 left-0 z-[65] w-80 border-r border-white/5 flex flex-col bg-sidebar-dark transition-transform duration-300 lg:relative lg:translate-x-0
        ${showRecentChats ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
      `}>
        <div className="p-6 pt-20 lg:p-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              onClick={() => setShowRecentChats(false)}
              className="lg:hidden w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-text-muted"
            >
              <X size={20} />
            </button>
          </div>
      ...
      <div className="flex-1 flex flex-col relative min-w-0">
        {/* Header Tabs */}
        <div className="p-4 lg:p-6 border-b border-white/5 flex items-center justify-between lg:justify-start gap-4 lg:gap-8">
          <div className="flex items-center gap-4 lg:gap-8">
            <button
              onClick={() => setActiveTab('nutrition')}
              className={`text-[10px] lg:text-sm font-black uppercase tracking-widest transition-colors ${
            ...
            <div className="relative">
              <div className="absolute left-3 lg:left-4 top-1/2 -translate-y-1/2">
                <button
                  ...
                  disabled={isTyping}
                  className={`w-9 h-9 lg:w-10 lg:h-10 rounded-xl bg-white/5 flex items-center justify-center transition-colors ${
              ...
              <input
                ...
                placeholder={selectedImage ? 'Add a note...' : 'Ask anything...'}
                disabled={isTyping}
                className={`w-full bg-surface-dark border border-white/10 rounded-2xl py-4 lg:py-6 pl-14 lg:pl-16 pr-14 lg:pr-16 text-xs lg:text-sm transition-colors ${
              ...
              <div className="absolute right-3 lg:right-4 top-1/2 -translate-y-1/2">
                <motion.button
                  ...
                  className={`w-9 h-9 lg:w-10 lg:h-10 rounded-xl flex items-center justify-center transition-all ${
