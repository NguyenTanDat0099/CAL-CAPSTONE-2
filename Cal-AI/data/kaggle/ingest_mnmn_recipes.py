...
CSV_PATH = _REPO_ROOT / "Cal-AI" / "data" / "dataset" / "food_dataset_mnmnweb.csv"

COLLECTION = "mnmn_recipes_768"
DOMAIN = "recipe"
...
def _build_text(r: dict) -> str:
    ...
    if r.get("ingredients"):
        parts.append(f"Nguyên liệu: {r['ingredients'][:250]}")
    if r.get("prep"):
        parts.append(f"Sơ chế: {r['prep'][:200]}")
    if r.get("instructions"):
        parts.append(f"Cách làm: {r['instructions'][:300]}")
    if r.get("tips"):
        parts.append(f"Mách nhỏ: {r['tips'][:150]}")
    if r.get("calories"):
        parts.append(f"Calo: {r['calories']} kcal")
    parts.append("nguồn: monngonmoingay.com")
    return " | ".join(parts)[:400]


def run():
    print("🚀 ingest_mnmn_recipes — MonNgonMoiNgay Vietnamese Recipes (768-dim)")
    ...
    for record in records:
        ...
        cache_key = f"mpnet768:mnmn:{text}"
    ...
    print(f"\n🎯 DONE ingest_mnmn_recipes → {total} records in '{COLLECTION}'")
